...
export function AdminDashboard({ admin, view = "overview" }: { admin: string; view?: AdminView }) {
  ...
  const metrics = (
    <section className="admin-metrics">
      ...
      <div><span>Live now</span><strong>{data?.analytics?.liveVisitors || 0}</strong></div>
      <div><span>Sessions</span><strong>{data?.analytics?.totalSessions || 0}</strong></div>
      <div><span>Tracked events</span><strong>{data?.analytics?.totalEvents || 0}</strong></div>
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <div>
          <span>NISHIT GAJJAR · FREELANCER ADMIN</span>
          <h1>Admin</h1>
          <small>{admin}</small>
        </div>
