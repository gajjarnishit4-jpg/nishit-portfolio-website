...
const host = "thefullstackguys.us";
...
const paths = ["/", "/about", "/process", "/pricing", "/support", "/privacy-policy", "/cookie-policy", "/terms-of-use", "/refund-policy", "/legal-notice", "/admin"];
let home="";
for (const path of paths) {
  ...
  assert.ok(html.includes(`https://thefullstackguys.us${path}`),`canonical missing for ${path}`);
  if (path==="/") home=html;
}
...
for (const path of ["/robots.txt","/sitemap.xml"]) {
  const response=await get(path);assert.equal(response.status,200,path);
  const text=await response.text();assert.match(text,/https:\/\/thefullstackguys\.us/);assert.doesNotMatch(text,legacy);
}
...
const www=await get("/","www.thefullstackguys.us");assert.equal(www.status,200);assert.match(await www.text(),/NISHIT GAJJAR/);
